    ...
    it 'does not erase what the other writer stored' do
      allow(ip_lookup).to receive(:perform) do
        Contact.find(contact.id).update!(
          additional_attributes: contact.additional_attributes.merge('company_name' => 'indicafacil.app')
        )
        geocoder_result
      end
      ...
      expect(contact.reload.additional_attributes).to include('company_name' => 'indicafacil.app', 'city' => 'Sao Paulo')
    end

    it 'does not resurrect a key the other writer removed' do
      contact.update!(additional_attributes: contact.additional_attributes.merge('company_name' => 'indicafacil.app'))
      allow(ip_lookup).to receive(:perform) do
